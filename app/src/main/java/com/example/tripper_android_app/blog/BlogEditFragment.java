...
public class BlogEditFragment extends Fragment {
    ...
    private CommonTask groupGet1Task, InsertNoteTask, getImageTask,blogGetAllTask;
    private TextView tvBlogName, tvDate, tvTime;
    private String startDate, tripId,startTime;
...
//取得前頁bungle
        ...
        String blogName = bundle.getString("BlogTitle");
        startDate = bundle.getString("BlogDate");
        startTime = bundle.getString("BlogTime");
        tripId = bundle.getString("BlogId");
        tvBlogName.setText(blogName);
        tvTime.setText("出發時間: "+ startTime);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                Navigation.findNavController(tvBlogName).navigate(R.id.action_blogEditFragment_to_blog_HomePage);
                return true;
            case R.id.btNextStep:
                Bundle bundle = new Bundle();
                bundle.putString("BlogId", tripId);
                findNavController(tvBlogName).navigate(R.id.action_blogEditFragment_to_blogEditFinishFragment, bundle);
                return true;
...
//創建Adapter
    private class BlogSpotAdapter extends RecyclerView.Adapter {
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            if (viewType == 1) {
                View itemView = layoutInflater.inflate(R.layout.item_view_editblog_spot, parent, false);
                return new BlogSpotAdapter.ViewHolderSpot(itemView);
            } else {
                View itemView = layoutInflater.inflate(R.layout.item_view_createblog_day, parent, false);
                return new BlogSpotAdapter.ViewHolderDay(itemView);
            }
        }
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
            ...
            if (position == 0 || position == day1count || position == day2count || position == day3count || position == day4count || position == day5count || position == day6count) {
                ...
                BlogSpotAdapter.ViewHolderDay viewHolderDay = (BlogSpotAdapter.ViewHolderDay) holder;
